...
--- @param args any[]
--- @return string
function SimpleSpy:ArgsToString(method, Fusion)
    assert(typeof(method) == "string", "string expected, got " .. typeof(method))
    assert(typeof(Fusion) == "table", "table expected, got " .. typeof(Fusion))
    return v2v({Fusion = Fusion}) .. "\n\n" .. method .. "(unpack(Fusion))"
end
...
function genScript(remote, ...)
    ...
    gen = ""
    Fusionscripts = {...}
    if #Fusionscripts > 0 then
        if not pcall(function()
                gen = v2v({Fusionscripts = Fusionscripts}) .. "\n"
            end)
        then
            ...
            if
                not pcall(
                    function()
                        for i, v in pairs(Fusionscripts) do
        ...
        if remote:IsA("RemoteEvent") then
            gen = gen .. v2s(remote) .. ":FireServer(unpack(Fusionscripts))"
        elseif remote:IsA("RemoteFunction") then
            gen = gen .. v2s(remote) .. ":InvokeServer(unpack(Fusionscripts))"
        end
    else
        if remote:IsA("RemoteEvent") then
...
--- @param t any
function v2v(t)
    topstr = "-- Script generated by Fusion Spy - discord: Fusionscripts"
    bottomstr = ""
    ...
    for i, v in pairs(t) do
        if type(i) == "string" and i:match("^[%a_]+[%w_]*$") then
            ret = ret .. "" .. i .. " = " .. v2s(v, nil, nil, i, true) .. "\n"
        elseif tostring(i):match("^[%a_]+[%w_]*$") then
            ret = ret .. "" .. tostring(i):lower() .. "_" .. tostring(count) .. " = " .. v2s(v, nil, nil, tostring(i):lower() .. "_" .. tostring(count), true) .. "\n"
        else
            ret = ret .. "" .. type(v) .. "_" .. tostring(count) .. " = " .. v2s(v, nil, nil, type(v) .. "_" .. tostring(count), true) .. "\n"
        end
        count = count + 1
    end
